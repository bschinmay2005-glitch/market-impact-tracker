                ...
                for entry in entries[:15]:
                    ...
                    if title not in st.session_state.seen_headlines:
                        ...
                        if analysis.get("significant"):
                            # Save to state so it stays on screen
                            st.session_state.market_news.insert(0, {
                                "title": title,
                                "link": link,
                                "provider": provider,
                                "analysis": analysis
...
# --- DISPLAY ALL SAVED NEWS ---
if not st.session_state.market_news:
    st.info("Scanner Warming Up... Waiting for economic signals.")
else:
    for item in st.session_state.market_news:
        ans = item['analysis']
        ...
        st.markdown(f"""
            <div style="border-left: {border_width} solid {color}; padding: 20px; background: #161b22; margin-bottom: 15px; border-radius: 12px; border: 1px solid #30363d;">
                ...
                <div style="display: flex; gap: 15px; margin-top: 10px;">
                    <a href="{item['link']}" target="_blank" style="color: #58a6ff; font-size: 12px; font-weight: bold; text-decoration: none;">READ SOURCE →</a>
